...
class GlobalContrastive(nn.Module):
    def __init__(self, margin=0.5, **kwargs):
        super(GlobalContrastive, self).__init__()
        self.margin = margin
    ...
    def forward(self, inputs, targets, global_inputs, global_targets, margin):
        ...
        for i in range(n):
            pos_pair_ = torch.masked_select(sim_mat[i], global_targets == targets[i])

            #  move itself
            pos_pair_ = torch.masked_select(pos_pair_, pos_pair_ < 1)
            neg_pair_ = torch.masked_select(sim_mat[i], global_targets != targets[i])
